...
> 如果 SecureBoot 处于关闭状态，脚本会自动跳过导入。
...
OOBE 创建用户后，fish、starship、ghostty、niri override 等配置文件已通过 `/etc/skel` 自动分发到用户目录，无需额外设置。

如需自定义 niri 配置，编辑 `~/.config/niri/override.kdl`（已预置模板）。
